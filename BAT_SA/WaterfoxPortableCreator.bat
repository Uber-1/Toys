@echo off
@title Waterfox Portable Creator - ver.2.4.2 [24.01.2024]
@cd /d "%~dp0"

@if not exist "curl.exe" @if not exist "%SystemRoot%\SYSTEM32\curl.exe" (
@echo Downloading with powershell . . .
@powershell -Command "$wc = New-Object System.Net.WebClient; $wc.Headers.Add('referer','https://download.mozilla.org'); $wc.DownloadFile('https://cdn1.waterfox.net/waterfox/releases/latest/windows', 'wfwin.exe.7z')"
@powershell -Command "(New-Object Net.WebClient).DownloadFile('https://www.7-zip.org/a/7zr.exe', '7zr.exe')"
) else (
@echo Downloading with CURL . . .
@curl.exe -RL# "https://cdn1.waterfox.net/waterfox/releases/latest/windows" -e"https://download.mozilla.org" -o "wfwin.exe.7z"
@curl.exe -RLO# "https://www.7-zip.org/a/7zr.exe"
)

@md "WaterfoxPortable\core\defaults\pref"
@md "WaterfoxPortable\core\distribution"
@"7zr.exe" x -t7z -bso0 "wfwin.exe.7z" -o"WaterfoxPortable" -xr!setup.exe -xr!crashreporter.exe -xr!default-browser-agent.exe -xr!maintenanceservice*.exe -xr!minidump-analyzer.exe -xr!updater.exe -xr!uninstall
...
@echo defaultPref^("browser.download.autohideButton", false^);
...
@echo defaultPref^("datareporting.sessions.current.activeTicks", 20^);
@echo defaultPref^("datareporting.sessions.current.firstPaint", 1285^);
@echo defaultPref^("datareporting.sessions.current.main", 2^);
@echo defaultPref^("datareporting.sessions.current.sessionRestored", 1424^);
@echo defaultPref^("datareporting.sessions.current.startTime", "1404409250562"^);
@echo defaultPref^("datareporting.sessions.current.totalTime", 96^);
@echo defaultPref^("datareporting.sessions.currentIndex", 5^);
...
@echo defaultPref^("font.size.fixed.x-cyrillic", 11^);
@echo defaultPref^("full-screen-api.warning.delay", 250^);
@echo defaultPref^("full-screen-api.warning.timeout", 1500^);
@echo defaultPref^("gecko.buildID", "20160823121617"^);
@echo defaultPref^("gecko.mstone", "48.0.2"^);
...
@echo defaultPref^("security.insecure_connection_text.enabled", true^);
...
(@echo @-moz-document domain^("youtube.com"^) {:root {scrollbar-width: none !important; /* thin/auto/none */} })>"WaterfoxPortable\portable\chrome\userContent.css"

(
@echo./* Left menu */
@echo.#PanelUI-button { -moz-box-ordinal-group:0 ; margin-inline-start:0px !important ; margin-inline-end:0px !important ; border-inline-start:none !important ; border-inline-end:0px solid !important ; }
@echo.
@echo./* Tab's fonts */
...
@echo.
@echo./*Basic Settings*/
@echo.:root { --navbarWidth:40vw ; --animationSpeed:0.15s ; } /* Set width of navbar. Use px for a fixed width or vw for a percentage of your window. */
...
@echo.#statuspanel-label { color:green !important; }
...
@echo user_pref^("browser.download.autohideButton", false^);
...
@echo user_pref^("datareporting.sessions.current.activeTicks", 20^);
@echo user_pref^("datareporting.sessions.current.firstPaint", 1285^);
@echo user_pref^("datareporting.sessions.current.main", 2^);
@echo user_pref^("datareporting.sessions.current.sessionRestored", 1424^);
@echo user_pref^("datareporting.sessions.current.startTime", "1404409250562"^);
@echo user_pref^("datareporting.sessions.current.totalTime", 96^);
@echo user_pref^("datareporting.sessions.currentIndex", 5^);
...
@echo user_pref^("font.size.fixed.x-cyrillic", 11^);
@echo user_pref^("full-screen-api.warning.delay", 250^);
@echo user_pref^("full-screen-api.warning.timeout", 1500^);
@echo user_pref^("gecko.buildID", "20160823121617"^);
@echo user_pref^("gecko.mstone", "48.0.2"^);
...
@echo user_pref^("security.insecure_connection_text.enabled", true^);
